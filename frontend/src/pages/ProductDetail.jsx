    ...
    <div>
      ...
      <div className="card" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '2rem' }}>
        <div style={{ background: 'var(--primary-light)', borderRadius: 'var(--radius)', display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '300px', fontSize: '5rem' }}>
          {product.image_url ? <img src={product.image_url} alt={product.product_name} style={{ maxHeight: '300px', borderRadius: 'var(--radius)' }} /> : '📦'}
        </div>
        <div>
          <span className="badge badge-primary" style={{ marginBottom: '0.75rem' }}>{product.category_name || 'Uncategorized'}</span>
          <h1 style={{ fontSize: '1.6rem', fontWeight: '700', marginBottom: '0.75rem' }}>{product.product_name}</h1>
          ...
          <div style={{ color: product.inventory_count > 0 ? 'var(--success)' : 'var(--danger)', marginBottom: '1.5rem', fontWeight: '600' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '1.5rem' }}>
            ...
            <div className="qty-control">
              ...
              <button className="qty-btn" onClick={() => setQty(Math.min(product.inventory_count, qty + 1))}>+</button>
            </div>
          </div>
